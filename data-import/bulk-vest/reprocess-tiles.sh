...
")
  ...
  read -r VOTING_COLS HAS_VAP HAS_CVAP HAS_ADJ ADJ_COLS < <(cd "$PROJECT_DIR" && python3 -c "
import json
...
for i in range(start, len(chunk)):
  ...
  if depth == 0:
    ...
    has_adj = 'yes' if adj else 'no'
    adj_str = ','.join(adj) if adj else ''
    print(','.join(sorted(vote_cols)), has_vap, has_cvap, has_adj, adj_str)
    break
" 2>/dev/null || echo " no no no ")
  VOTING_FLAGS=""
  if [[ -n "$VOTING_COLS" ]]; then
    VOTING_FLAGS="-v $VOTING_COLS"
  fi
  DEMO_FLAGS="-d population,white,black,asian,hispanic,other"
  if [[ "$HAS_ADJ" == "yes" && -n "$ADJ_COLS" ]]; then
    DEMO_FLAGS="$DEMO_FLAGS -d $ADJ_COLS"
